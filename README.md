...
## Server setup (simulation training)

The environment is reproduced via conda + pip:
...
`requirements-server.txt` is the clean, server-deployable dependency set (RL frameworks, JAX/PyTorch, vision libs). The older `requirements-lock.txt` is a full `pip freeze` from the development machine and is **not** installable on a clean server (it contains ROS2 and other system packages); keep it only as a reference snapshot.

GPU notes:
- JAX uses `jax-cuda12-plugin`; the server needs an NVIDIA driver compatible with CUDA 12.x.
- If `import dreamerv3` fails on a missing `configs.yaml`, the Pippo809 fork installed incompletely — see `CLAUDE.md`.

Run DreamerV3 simulation training:

```bash
python scripts/train_dreamer_game.py             # main by-the-wall sim
python scripts/train_dreamer_game_randomized.py  # randomized-env variant
```

Logs are written to `logdir/` inside the repo (override with the `DREAMER_GAME_LOGDIR` env var).
...
> **Note:** The demonstration GIFs (`results/*.gif`, ~170MB) are not included in this
> server training fork to keep clones lightweight. They are available in the original
> upstream repository:
> [M-Medany/Model-Based-Reinforcement-Learning-for-Ultrasound-Driven-Autonomous-Microrobots](https://github.com/M-Medany/Model-Based-Reinforcement-Learning-for-Ultrasound-Driven-Autonomous-Microrobots).
