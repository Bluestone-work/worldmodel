...
def main():
    truncation = 100
    path = pathlib.Path(LOGDIR).absolute().resolve()
    os.makedirs(path, exist_ok=True)
    assert "PPO" in EXP_NAME, "Change experiment name, must contain PPO"

    experiment_dir = pathlib.Path(path, EXP_NAME)
    suffix = 1
    while experiment_dir.exists():
        experiment_dir = pathlib.Path(path, f"{EXP_NAME}_{suffix}")
        suffix += 1
    ...
    def wrap_env(env, eval="train"):
        ...
        env = RateTargetReachedWrapper(env, 100, logdir=f"{experiment_dir}/rate_target_reached_{eval}.csv")
        env = RewardWrapper(env, logdir=f"{experiment_dir}/reward_{eval}.csv")
        env = NewApi(env)
    ...
    callback = CallbackList([
        CheckpointCallback(save_freq=100000, save_path=f"{experiment_dir}", name_prefix=experiment_name),
        EvalCallback(eval_env, best_model_save_path=f"{experiment_dir}", log_path=f"{experiment_dir}", eval_freq=2000, deterministic=True, render=True,
                     n_eval_episodes=25
                     ),
    ])
    ...
    with open(config_path, 'w') as stream:
        try:
            ...
            print(exc)
    with open(experiment_dir / "env_config.yaml", 'w') as stream:
        try:
    ...
    if ckp != "":
        ...
        model = PPO("MultiInputPolicy", env, verbose=1, tensorboard_log=f"{experiment_dir}", device="cuda")

    model.learn(tb_log_name=experiment_name, total_timesteps=100e6, progress_bar=True, callback=callback, reset_num_timesteps=False)
